...
for i=1301:1:1301
    ...
    %for j=1:1:gt(i).num
        ...
        NOTFN = 0;
        A = [gt(i).rect(1,1) gt(i).rect(1,2) gt(i).rect(1,3) gt(i).rect(1,4)];
        G_area = rectint(A,A);
    for k=1:1:size(bbox,1)
            ...
            f = extractHOGFeatures(cim);
            %figure; imshow(cim);
            [car_class_new, car_score_new] = predict(SVMModel,f(:,:));
            if(car_class_new == 1)
                %figure;
                count = count + 1;
                %detectedImg_HOG_SVM = insertObjectAnnotation(im, 'rectangle', B, 'HOG_SVM');
                C(count,:)= B;
                %imshow(detectedImg_HOG_SVM);
            end
    ...
    detectedImg_HOG_SVM = insertObjectAnnotation(im, 'rectangle', C, 'HOG_SVM');
    figure; imshow(detectedImg_HOG_SVM)
    %% TP
    for j=1:1:gt(i).num
        NOTFN = 0;
